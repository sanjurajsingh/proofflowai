...
-- Allow users to update their own wallet_address via the existing protect trigger:
-- wallet_address is NOT a protected finance field, so it passes through.
...
-- Treasury transactions (fund / reserve / release / spend)
DO $$ BEGIN
...
-- updated_at trigger for treasuries
